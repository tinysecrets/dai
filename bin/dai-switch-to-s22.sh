...
systemctl --user status audiosource-s22.service s22-mic-router.service --no-pager -n 4 2>&1 || true

echo ""
echo "Testing 2-second capture from S22 microphone (android-4f3b250)..."
timeout 2s parec --device="android-4f3b250" --raw > /tmp/dai-s22-test.raw 2>/dev/null || true
